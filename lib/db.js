...
/** True when a hosted database is configured, rather than the dev-only file. */
export const hasHostedDb = () => Boolean(process.env.DATABASE_URL);
 ...
 * visitor must never see an error page because the database is down or not yet
 * configured. Only the personalised extras — ratings, ownership — degrade.
 * Writes never use this: a failed purchase has to fail loudly.
 */
export async function readOrFallback(fn, fallback) {
  try {
    ...
    if (!warned) {
      warned = true;
      console.warn(
        `[db] unavailable, serving catalogue only: ${err.message}. ` +
          `Set DATABASE_URL (see README) to enable accounts and orders.`
      );
    }
    return fallback;
  }
